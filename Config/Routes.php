...
if (strpos($dirName, '_') === false) {
    ...
    $mdm = model("App\Modules\Organization\Models\Organization_Modules");
    $mdcxm = model("App\Modules\Organization\Models\Organization_Clients_Modules");
    $module = 'organization';
    ...
    if ($cxm == "authorized") {
        $routes->group($module,
            ['namespace' => $namespace],
            function ($subroutes) {
                $subroutes->add('/', 'Organization::index');
                $subroutes->add('/home', 'Organization::index');
                $subroutes->add('home/(:any)', 'Organization::home/$1');
        ...
        $routes->group($module,
            ['namespace' => $namespace],
            function ($subroutes) {
                $subroutes->add('/', 'Organization::denied');
                $subroutes->add('(:any)', 'Organization::denied');
            }
        );
    }
}
