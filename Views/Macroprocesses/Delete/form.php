...
$r = $model->where('macroprocess', $oid)->first();
$name = urldecode($r["name"]);
$message = sprintf(lang("Macroprocesses.delete-message"), $name);
...
$card = $bootstrap->get_Card("delete-{$oid}", array(
    "class" => "card-info",
    "icon" => ICON_WARNING,
    "title" => sprintf(lang("Macroprocesses.delete-title"), $name),
    "text-class" => "text-center",
    "text" => sprintf(lang("Macroprocesses.delete-message"), $name),
    "content" => $f,
    "header-back" => $back
));
echo($card);
