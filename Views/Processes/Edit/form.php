...
//[vars]----------------------------------------------------------------------------------------------------------------
$row = $mprocesses->where('process', $oid)->first();
$r["process"] = $f->get_Value("process", $row["process"]);
$r["macroprocess"] = $f->get_Value("macroprocess", $row["macroprocess"]);
...
$back = "/organization/processes/list/" . $row["macroprocess"];
...
//[groups]----------------------------------------------------------------------------------------------------------------
$f->groups["g1"] = $f->get_Group(array("legend" => "", "fields" => ($f->fields["process"] . $f->fields["macroprocess"] . $f->fields["responsible"])));
...
//[build]---------------------------------------------------------------------------------------------------------------
$macroproccess = $mmacroprocesses->get_Macroprocess($row["macroprocess"]);
$message = $macroproccess['description'];
$title = $macroproccess['name'];
$card = $bootstrap->get_Card("create", array(
    "title" => lang("Processes.edit-title"),
