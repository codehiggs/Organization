...
$r = $model->where('subprocess', $oid)->first();
$name = urldecode($r["name"]);
$message = sprintf(lang("Subprocesses.delete-message"), $name);
...
$card = $bootstrap->get_Card("delete-{$oid}", array(
    "class" => "card-info",
    "icon" => ICON_WARNING,
    "title" => sprintf(lang("Subprocesses.delete-title"), $name),
    "text-class" => "text-center",
    "text" => sprintf(lang("Subprocesses.delete-message"), $name),
    "content" => $f,
    "header-back" => $back
));
echo($card);
